        ...
        for (let y in t) {
            ...
            for (let x in t[y]) {
                ...
                if (Number.isInteger(i) || i != "") {
                    ...
                    h2 += `<img class='img_desc' id="sc_upg_${id}${i}_max_base" draggable="false" src="${"images/max.png"}">
                        <img class="img_res" draggable="false" src="${"images/"+SC_ICONS[id][0]+".png"}"><img class="img_res" draggable="false" src="${"images/"+SC_ICONS[id][1]+".png"}">
...
function updateStarChart() {
    ...
    if (ch[0]) {
        ...
        let canBuy = Decimal.gte(res, tt.cost[i])
        ...
        if (amt < tt.max[i]) {
            ...
            if (hasBuy25) h += `<span class="${Decimal.gte(res,cost2)?"green":"red"}">Cost to next 25: ${format(cost2,0)} ${resDisplay}</span><br>`
            else if (hasMax) h += `<span class="${Decimal.gte(res,cost3)?"green":"red"}">Cost to max: ${format(cost2,0)} ${resDisplay}</span><br>`

            h += `<span class="${Decimal.gte(res,tt.cost[i])?"green":"red"}">Cost: ${format(tt.cost[i],0)} ${resDisplay}</span>`
    ...
    for (let id in STAR_CHART) {
        ...
        if (d) for (let i = 0; i < STAR_CHART[id].length; i++) {
            ...
            if (id2) {
                ...
                tmp.el[id2+"_cost"].setTxt(format(tt.cost[i],0,6))
